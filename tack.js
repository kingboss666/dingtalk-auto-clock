 ...
 * @LastEditTime: 2021-11-05 09:18:06
...
const OBSERVE_VOLUME_KEY = false
...
function notificationHandler(n) {
  ...
  var abstract = n.title || n.tickerText        // 获取通知摘要
...
function clockIn() {
  ...
  if (null != textMatches("上班打卡").clickable(true).findOne(1000)) {
    ...
    click(device.width / 2, device.height * 0.60)
 ...
 * @description 监听结果
 */
function monitoringResults(text) {
  if (null != textContains("打卡成功").findOne(1000)) {
    setStorageData("dingding", "clockResult", text)
    threads.shutDownAll()
    threads.start(function () {
      sendQQMsg(text)
